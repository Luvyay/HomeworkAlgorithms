...
public class BinTree<T extends Comparable<T>> {
    ...
    public boolean add(T _value){
        ...
        if (addNode(root, _value) != null){
            return true;
        }
        return false;
    }

    private Node addNode(Node node, T _value){
        ...
        if (node.value.compareTo(_value) > 0) {
            ...
            Node resultNode = addNode(node.left, _value);
            ...
            Node resultNode = addNode(node.right, _value);
    ...
    public boolean remove(T _value) {
        ...
        while (deleteNode != null) {
            if (deleteNode.value.compareTo(_value) == 0) {
            ...
            if (prevNode != deleteNode) {
                if (prevNode.value.compareTo(_value) > 0)
    ...
    private boolean contain(T _value) {
        Node currentNode = root;
        while (currentNode != null) {
            if (currentNode.value.equals(_value)) {
                return true;
            }
            if (currentNode.value.compareTo(_value) > 0) {
                currentNode = currentNode.left;
            } else {
